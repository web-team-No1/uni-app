	...
	<view>
		...
		<view class="typeQueryNav">
			<view class="typeQueryBack">
				...
				<text class="typeQueryTitle">待报修</text>
			</view>
		...
		<view class="tbTh">
			...
			<view style="width: 10%;" class="tbThclass">保修期</view>
			<view style="width: 15%;" class="tbThclass">分配时间</view>
			<view style="width: 10%;" class="tbThclass">保修人员</view>
			<view style="width: 15%;" class="tbThclass">操作</view>
		</view>
		<view v-for="(j,index) in datas" :key="index" :class="['tbTd',index==datas.length - 1? 'notbTd':'']">
			...
			<view style="width: 10%;" name="联系电话" class="tbTdclass">{{j.sex}}</view>
			...
			<view style="width: 15%;" name="是否加急" class="tbTdclass">{{j.typeTime}}</view>
			<view style="width: 10%;" name="X光片" class="tbTdclass">{{j.state}}</view>
			<view style="width: 15%;" name="操作" class="tbTdclass">
...
<script>
	...
	export default {
		...
		methods: {
			...
			typeQueryReception() {
				uni.navigateTo({
					url: "../readyService/redServicedetial"
				})
			}
		},
		onLoad(option) {}
